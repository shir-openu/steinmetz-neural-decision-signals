...
# Original layout: A, B, C in one row
fig, axes = plt.subplots(1, 3, figsize=(15, 5))

# Plot A: Accuracy over time (left)
ax1 = axes[0]
...
ax1.set_ylim(45, 72)
# Removed 'Stimulus Onset' text - will note in legend instead
ax1.set_xticks(x_pos)
ax1.set_xticklabels(time_labels, fontsize=14)
ax1.set_ylabel('Decoding Accuracy (%)', fontsize=14)
ax1.set_title('A. Choice Decoding Across Time', fontsize=16)
...
# Plot B: Pre-stimulus comparison (middle)
ax2 = axes[1]
...
# Add p-value in title (no line needed)
if len(pre_inter_all) > 5 and len(pre_local_all) > 5:
    ...
    ax2.set_title(f'B. Pre-stimulus Period\n(-250 to 0ms)\n{sig_text}', fontsize=16)

# Plot C: Post-stimulus comparison (right)
ax3 = axes[2]
...
if len(post_inter_all) > 5 and len(post_local_all) > 5:
    ...
    sig_text = f'p = {p_val:.3f}' if p_val >= 0.001 else 'p < 0.001'
    ax3.set_title(f'C. Post-stimulus Period\n(0 to +250ms)\n{sig_text}', fontsize=16)

plt.tight_layout()
fig_path = os.path.join(script_dir, '..', 'FIGURES', 'pre_vs_post_analysis_v2.png')
